...
class PatientService extends BaseService<any> implements IBaseService<IPatient, CreatePatientDTO, UpdatePatientData> {
    ...
    async searchPatients(filters: PatientSearchFilters): Promise<PatientSearchResult> {
        ...
        const [patients, total] = await Promise.all([
            Patient.find(query).sort(sortOptions).skip(skip).limit(limit).populate('clinic', 'name'),
    ...
    async getPatientsByClinic(clinicId: string, status: 'active' | 'inactive' | 'all' = 'active'): Promise<IPatient[]> {
        ...
        return await Patient.find(query).sort({ name: 1 }).populate('clinic', 'name');
    }
    ...
    async getPatientStats(clinicId: string): Promise<{
        ...
        const [total, active, inactive, recentlyAdded, withMedicalHistory] = await Promise.all([
            ...
            Patient.countDocuments({ clinic: clinicId, createdAt: { $gte: thirtyDaysAgo } }),
    ...
    async reactivatePatient(patientId: string, clinicId: string): Promise<IPatient | null> {
        ...
        const patient = await Patient.findOne({ _id: patientId, clinic: clinicId, status: 'inactive' });
        if (!patient) {
