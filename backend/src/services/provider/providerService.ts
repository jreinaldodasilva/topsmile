...
class ProviderService extends BaseService<any> {
    ...
    async createProvider(data: CreateProviderData): Promise<IProvider> {
        ...
        if (data.userId) {
            const user = await User.findOne({ _id: data.userId, clinic: data.clinicId, isActive: true });
            if (!user) {
    ...
    async getProviderById(providerId: string, clinicId: string): Promise<IProvider | null> {
        ...
        const provider = await Provider.findOne({ _id: providerId, clinic: clinicId })
    ...
    async getProvidersByClinic(clinicId: string, isActive: boolean = true): Promise<IProvider[]> {
        ...
        return await Provider.find({ clinic: clinicId, isActive })
            .sort({ name: 1 })
            .populate('clinic', 'name')
            .populate('user', 'name email role')
            .populate('appointmentTypes', 'name duration color category');
    ...
    async getProviderStats(clinicId: string): Promise<{
        ...
        const [total, active, inactive, withSystemAccess, recentlyAdded, specialtyStats] = await Promise.all([
            ...
            Provider.countDocuments({ clinic: clinicId, user: { $exists: true, $ne: null } }),
            Provider.countDocuments({ clinic: clinicId, createdAt: { $gte: thirtyDaysAgo } }),
            Provider.aggregate([
