...
expect.extend({
    toHaveLoadingState(received: HTMLElement) {
        ...
        const hasLoadingClass = received.classList.contains('loading') || received.querySelector('.loading') !== null;
        const hasSpinner =
            received.querySelector('[role="progressbar"]') !== null || received.querySelector('.spinner') !== null;
...
expect.extend({
    toHaveErrorState(received: HTMLElement) {
        const hasErrorText =
            received.textContent?.includes('Erro') ||
            received.textContent?.includes('Error') ||
            received.querySelector('[data-testid*="error"]') !== null;
        const hasErrorClass = received.classList.contains('error') || received.querySelector('.error') !== null;
        const hasErrorRole =
            received.getAttribute('role') === 'alert' || received.querySelector('[role="alert"]') !== null;
