...
const AllTheProviders = ({
    ...
    let component = <BrowserRouter>{children}</BrowserRouter>;

    if (withError) {
        component = <ErrorProvider>{component}</ErrorProvider>;
    }
    ...
    if (withPatientAuth) {
        component = <PatientAuthProvider>{component}</PatientAuthProvider>;
    }
...
const customRender = (ui: ReactElement, options: CustomRenderOptions = {}): RenderResult => {
    const { withAuth = true, withPatientAuth = false, withError = true, ...renderOptions } = options;

    const Wrapper = ({ children }: { children: React.ReactNode }) => (
        <AllTheProviders withAuth={withAuth} withPatientAuth={withPatientAuth} withError={withError}>
            {children}
        </AllTheProviders>
    );
