...
export const handlers = [
    ...
    http.post(`${API_BASE}/api/auth/login`, async ({ request }) => {
        ...
        return HttpResponse.json(generateMockErrorResponse('E-mail ou senha inválidos', 401), { status: 401 });
    }),
    ...
    http.get(`${API_BASE}/api/auth/me`, ({ request }) => {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
            return HttpResponse.json(generateMockErrorResponse('Token não fornecido', 401), { status: 401 });
        }
    ...
    http.post(`${API_BASE}/api/auth/logout`, async () => {
        return HttpResponse.json(generateMockApiResponse(null, true, 'Logout realizado com sucesso'), { status: 200 });
    }),
    ...
    http.get(`${API_BASE}/api/patients/:id`, ({ params }) => {
        ...
        if (!patient) {
            return HttpResponse.json(generateMockErrorResponse('Paciente não encontrado', 404), { status: 404 });
        }
    ...
    http.all('*', ({ request }) => {
        ...
        return HttpResponse.json(generateMockErrorResponse('Endpoint não encontrado', 404), { status: 404 });
    })
];
