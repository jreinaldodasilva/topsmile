...
jest.mock('../../components/Clinical/DentalChart', () => ({
    DentalChart: ({ patientId }: { patientId: string }) => (
        <div data-testid="dental-chart">DentalChart: {patientId}</div>
    )
}));
...
jest.mock('../../components/Clinical/MedicalHistory', () => ({
    MedicalHistoryForm: ({ patientId }: { patientId: string }) => (
        <div data-testid="medical-history">History: {patientId}</div>
    )
}));
...
describe('PatientDetail Integration Tests', () => {
    ...
    test('saves patient updates', async () => {
        ...
        await waitFor(() => {
            expect(apiService.patients.update).toHaveBeenCalledWith(
                'patient-123',
