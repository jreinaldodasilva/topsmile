...
process.env.REACT_APP_VERSION = process.env.REACT_APP_VERSION || '1.0.0-test';
...
beforeAll(() => {
    console.error = (...args: any) => {
        if (typeof args[0] === 'string' && args[0].includes('Warning:') && args[0].includes('ReactDOMTestUtils')) {
            return;
        }
