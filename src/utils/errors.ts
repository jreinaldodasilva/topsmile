...
export const handleApiError = (error: any): Error => {
    ...
    switch (status) {
        ...
        case 400:
            return new ValidationError(data?.error?.message || 'Dados inválidos', data?.error?.details);
        default:
            return new ApiError(data?.error?.message || 'Erro ao processar requisição', status, data?.error?.code);
    }
};
