...
const ProviderForm: React.FC<ProviderFormProps> = ({ provider, onSave, onCancel, loading = false }) => {
    ...
    const handleAppointmentTypeChange = (type: string, checked: boolean) => {
        setFormData(prev => ({
            ...prev,
            appointmentTypes: checked ? [...prev.appointmentTypes, type] : prev.appointmentTypes.filter(t => t !== type)
        }));
    };
