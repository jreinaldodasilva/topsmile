...
const PatientNavigation: React.FC<PatientNavigationProps> = ({ activePage }) => {
    ...
    const getActivePage = () => {
        ...
        if (path === '/patient/medical-records') return 'medical-records';
    ...
    const navigationLinks = (
        <>
            ...
            <button
                className={`nav-link ${currentActivePage === 'medical-records' ? 'active' : ''}`}
                onClick={() => navigate('/patient/medical-records')}
                aria-label="Ir para Histórico Médico"
                aria-current={currentActivePage === 'medical-records' ? 'page' : undefined}
            >
                <svg className="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                        strokeLinecap="round"
            ...
            <button
                className={`nav-link ${currentActivePage === 'prescriptions' ? 'active' : ''}`}
                onClick={() => navigate('/patient/prescriptions')}
                aria-label="Ir para Receitas"
                aria-current={currentActivePage === 'prescriptions' ? 'page' : undefined}
            >
                <svg className="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                        strokeLinecap="round"
    ...
    return (
        <nav className="patient-navigation">
            ...
            {isMobileMenuOpen && <div className="nav-links-mobile">{navigationLinks}</div>}
        </nav>
    );
};
