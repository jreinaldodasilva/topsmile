...
export const TreatmentPlanBuilder: React.FC<TreatmentPlanBuilderProps> = ({
  ...
  const [phases, setPhases] = useState<Phase[]>(initialData?.phases || [
    { phaseNumber: 1, title: 'Fase 1', procedures: [], status: 'pending' }
  ]);
  ...
  const addProcedure = (phaseNumber: number, procedure: Procedure) => {
    setPhases(phases.map(p => 
      p.phaseNumber === phaseNumber 
        ? { ...p, procedures: [...p.procedures, procedure] }
        : p
    ));
  };
  ...
  const removeProcedure = (phaseNumber: number, index: number) => {
    setPhases(phases.map(p => 
      p.phaseNumber === phaseNumber
        ? { ...p, procedures: p.procedures.filter((_, i) => i !== index) }
        : p
    ));
  };
  ...
  const totalCost = phases.reduce((sum, phase) => 
    sum + phase.procedures.reduce((pSum, proc) => pSum + proc.cost, 0), 0
  );

  const totalInsurance = phases.reduce((sum, phase) => 
    sum + phase.procedures.reduce((pSum, proc) => pSum + (proc.insuranceCoverage || 0), 0), 0
  );
  ...
  return (
    <div className="treatment-plan-builder">
      ...
      <div className="builder-actions">
        ...
        <button className="propose-btn" onClick={() => {
