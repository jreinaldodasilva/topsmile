...
export const MedicalHistoryForm: React.FC<MedicalHistoryFormProps> = ({ patientId, onSave, initialData }) => {
    ...
    useEffect(() => {
        request('/api/medical-history/conditions/medical').then(res =>
            res.ok && res.data ? setMedicalConditions(res.data) : null
        );

        request('/api/medical-history/conditions/dental').then(res =>
            res.ok && res.data ? setDentalConditions(res.data) : null
        );
    }, []);
    ...
    return (
        <div className="medical-history-form">
            ...
            <AllergyManager allergies={allergies} onChange={setAllergies} />

            <MedicationManager medications={medications} onChange={setMedications} />
