    ...
    <div className="allergy-manager">
      ...
      {allergies.length > 0 && (
        <div className="allergy-list">
          {allergies.map((allergy, idx) => (
            <div key={idx} className="allergy-item" style={{ borderLeftColor: getSeverityColor(allergy.severity) }}>
              <div className="allergy-info">
                <strong>{allergy.allergen}</strong>
                <span className="allergy-reaction">{allergy.reaction}</span>
                <span className={`severity-badge ${allergy.severity}`}>
                  {allergy.severity === 'mild' ? 'Leve' : allergy.severity === 'moderate' ? 'Moderada' : 'Grave'}
                </span>
              </div>
              <button onClick={() => removeAllergy(idx)} className="remove-allergy">×</button>
            </div>
          ))}
        </div>
      )}
      ...
      {showForm ? (
        <div className="allergy-form">
          ...
          <button onClick={addAllergy} className="add-btn">Adicionar</button>
