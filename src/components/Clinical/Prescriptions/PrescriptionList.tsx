...
export const PrescriptionList: React.FC<PrescriptionListProps> = ({ prescriptions, onView, onEdit, onDelete }) => {
                    ...
                    >
                        <div>
                            ...
                            <p style={{ color: '#666', margin: '5px 0' }}>
                                {new Date(rx.prescribedDate).toLocaleDateString('pt-BR')} - Dr.{' '}
                                {rx.provider?.name || 'N/A'}
                            </p>
                            <span
                                style={{
                                    padding: '4px 8px',
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <Button variant="outline" size="sm" onClick={() => onView(rx._id)}>
                                Ver
                            </Button>
                            <Button variant="outline" size="sm" onClick={() => onEdit(rx)}>
                                Editar
                            </Button>
                            <button
