...
export const TemplateSelector: React.FC<TemplateSelectorProps> = ({ noteType, onSelect }) => {
    ...
    useEffect(() => {
        request(`/api/clinical-notes/templates/type/${noteType}`).then(res =>
            res.ok && res.data ? setTemplates(res.data) : null
        );
    }, [noteType]);
    ...
    return (
        <div className="template-selector">
            <button onClick={() => setShowDropdown(!showDropdown)} className="template-btn">
                📋 Usar Template
            </button>
                ...
                <div className="template-dropdown">
                    {templates.length === 0 ? (
                        ...
                        templates.map(template => (
                            <div key={template.id} onClick={() => handleSelect(template.id)} className="template-item">
                                {template.name}
                            </div>
                        ))
                    )}
